...
+ (NSArray *)getDataFromString:(NSString *)string {
  ...
  for (int mediaItemRank = 0; mediaItemRank < NUMBER_OF_MEDIA_ITEMS; mediaItemRank++) {
    NSNumber *rank = [[NSNumber alloc] initWithInt:mediaItemRank];
    ITunesMediaItem *newMediaItem = [[ITunesMediaItem alloc] initWithJSONAttributes:parsedJSON[@"feed"][@"entry"][mediaItemRank] rank:rank];
