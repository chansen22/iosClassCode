...
- (void)viewWillAppear:(BOOL)animated {
  ITunesFavorites *favorite = [ITunesFavorites sharedFavoritesManager];
  self.favoriteButton.selected = [favorite isFavorite:self.mediaItem];
}
...
- (void)viewDidLoad
{
  ...
  self.rankLabel.text = [[NSString alloc] initWithFormat:@"%d", [self.mediaItem.rank integerValue] + 1];
...
- (IBAction)favorite:(UIButton *)sender {
  ITunesFavorites *favorite = [ITunesFavorites sharedFavoritesManager];
  if ([favorite isFavorite:self.mediaItem]) {
    [favorite removeFavorite:self.mediaItem];
  } else {
    [favorite addFavorite:self.mediaItem];
  }
  self.favoriteButton.selected = [favorite isFavorite:self.mediaItem];
}
